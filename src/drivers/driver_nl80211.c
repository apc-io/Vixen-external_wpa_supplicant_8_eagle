...
#define RSSI_ALGORITHM_TRIVIAL_PARAM 4
	...
	for(i = 0; i < res_cache->num; i++) {
		...
		if ( res_cache->res[i] && !os_memcmp(bssid, res_cache->res[i]->res->bssid, ETH_ALEN)) {
			...
			if (level > cached_ap_level)
				...
				res->level = ( cached_ap_level * (RSSI_ALGORITHM_TRIVIAL_PARAM - 1) + level ) / RSSI_ALGORITHM_TRIVIAL_PARAM;
...
static void maintain_res_cache(size_t *actual_num)
{
	...
	for( i = 0; i < res_cache->num; i++ ) {
		...
		if (!(res_cache->res[i])) {
			(*actual_num)--;
			wpa_printf(MSG_ERROR, "actual_num turns to %d, A", *actual_num);
			continue;
		}
