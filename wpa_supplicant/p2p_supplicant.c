					   ...
					   const u8 *go_dev_addr)
{
	...
	if (ssid->frequency) {
		s->frequency = ssid->frequency;
		wpa_printf(MSG_DEBUG, "==========================> save group frequency = %d", s->frequency);
	}
				  ...
				  int freq, int ht40)
{
	...
	int saved_freq = 0;
        for (s = wpa_s->conf->ssid; s; s = s->next) {
        if (os_memcmp(ssid->bssid, s->bssid, ETH_ALEN) == 0 &&
            s->ssid_len == ssid->ssid_len &&
            os_memcmp(ssid->ssid, s->ssid, ssid->ssid_len) == 0) {
            saved_freq = s->frequency;
            wpa_printf(MSG_ERROR, "======================>  get freq from config,"
                " ssid:%s, freq:%d, bssid:",s->ssid, saved_freq, MACSTR, MAC2STR(ssid->bssid));
		}
    }
	if (saved_freq == 0 || saved_freq < 5000)
		saved_freq = freq;
	wpa_printf(MSG_ERROR, "======================>  freq = %d", saved_freq);
    //===================

	if (wpas_p2p_init_go_params(wpa_s, &params, saved_freq, ht40))
		return -1;
